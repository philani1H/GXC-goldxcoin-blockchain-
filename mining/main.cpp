...
void printBanner() {
  ...
  / __| | \| | / __|   |  \/  |(_) _ _   ___  _ _ 
 | (__  | .` || (__    | |\/| || || ' \ / -_)| '_|
  \___| |_|\_| \___|   |_|  |_||_||_||_|\___||_|  
                                                 
GXC Multi-Algorithm Miner - Full Traceability Support
Version 1.0.0
    )" << std::endl;
}

void printHelp() {
    std::cout << "GXC Miner - Usage:" << std::endl;
    std::cout << "  gxc-miner [options]" << std::endl;
    std::cout << std::endl;
    std::cout << "Options:" << std::endl;
    std::cout << "  --help, -h                 Show this help message" << std::endl;
    std::cout << "  --address=ADDR, -a ADDR    Miner address (required)" << std::endl;
    std::cout << "  --algorithm=ALGO           Mining algorithm (sha256|ethash|gxhash)" << std::endl;
    ...
    std::cout << "  sha256     - Standard SHA256 mining" << std::endl;
    std::cout << "  ethash     - Ethash algorithm" << std::endl;
    std::cout << "  gxhash     - GXC custom algorithm with traceability optimization" << std::endl;
    std::cout << std::endl;
    std::cout << "Examples:" << std::endl;
    std::cout << "  gxc-miner -a gxc1abcd... --algorithm=gxhash" << std::endl;
    std::cout << "  gxc-miner -a gxc1abcd... --pool=stratum+tcp://pool.gxc.io:4444 --username=miner1" << std::endl;
    std::cout << std::endl;
}

struct MinerConfig {
    std::string minerAddress;
    std::string algorithm = "gxhash";
    std::string poolUrl;
...
bool parseCommandLine(int argc, char* argv[], MinerConfig& config) {
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            config.algorithm = arg.substr(12);
        }
...
bool validateConfig(const MinerConfig& config) {
    ...
    if (config.algorithm != "sha256" && config.algorithm != "ethash" && config.algorithm != "gxhash") {
        std::cerr << "Error: Invalid algorithm. Use sha256, ethash, or gxhash" << std::endl;
        return false;
    }
...
MiningAlgorithm stringToAlgorithm(const std::string& algo) {
    if (algo == "sha256") return MiningAlgorithm::SHA256;
    if (algo == "ethash") return MiningAlgorithm::ETHASH;
    if (algo == "gxhash") return MiningAlgorithm::GXHASH;
    return MiningAlgorithm::GXHASH; // Default
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        g_miningManager->setMiningAlgorithm(stringToAlgorithm(config.algorithm));
    ...
    LOG_MINING(LogLevel::INFO, "GXC Miner shutdown complete");
