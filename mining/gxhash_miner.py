...
class GXCMiner:
    ...
    def __init__(self, root):
        ...
        self.total_earned = 0.0  # Track total earnings from blockchain
        ...
        def connect():
            try:
                ...
                if info:
                    ...
                    self.root.after(0, lambda: self.status_label.config(text="● Connected", fg="#86efac"))
                ...
                self.node_connected = False
                self.root.after(0, lambda: self.status_label.config(text="● Disconnected", fg="#fca5a5"))
                self.root.after(0, lambda: self.log(f"⚠️ Auto-connect failed: {e}", "WARNING"))
    ...
    def refresh_transactions(self):
        ...
        def fetch():
            try:
                ...
                self.root.after(0, lambda b=balance: self.log(f"💵 Balance from blockchain: {b:.8f} GXC", "INFO"))
                ...
                self.root.after(0, lambda txs=transactions: self.log(f"📊 Loaded {len(txs)} transactions", "INFO"))
                self.root.after(0, lambda te=total_earned: self.log(f"💰 Total earned (from blockchain): {te:.8f} GXC", "INFO"))
            except Exception as e:
    ...
    def add_transaction_to_list(self, tx: Dict):
        ...
        if confirmations == 0 and (tx.get('block_number') or tx.get('block')):
            ...
            if tx_block > 0 and current_height > 0:
    ...
    def mine_thread(self, thread_id: int):
        ...
        while self.mining and not self.shutdown:
            ...
            if self.check_difficulty(hash_result, work['bits']):
                ...
                # Store reward for lambda closure
                final_reward = reward_amount
                final_hash = hash_result
                final_height = work['height']

                self.root.after(0, lambda: self.log(f"🎉 Block found by thread {thread_id}!", "SUCCESS"))
                self.root.after(0, lambda h=final_hash: self.log(f"   Hash: {h[:32]}...", "INFO"))
                if final_reward > 0:
                    self.root.after(0, lambda r=final_reward: self.log(f"   Reward: {r} GXC (from blockchain)", "SUCCESS"))
                ...
                if self.wallet_address.get() and final_reward > 0:
                    coinbase = {
                        'type': 'coinbase',
                ...
                try:
                    ...
                    self.root.after(0, lambda err=str(e): self.log(f"❌ Block submission error: {err}", "ERROR"))
                    tx_hash = None
                ...
                if tx_hash:
                    ...
                    if final_reward > 0:
                        self.root.after(0, lambda r=final_reward: self.log(f"💰 Expected reward: {r} GXC (will verify from blockchain)", "INFO"))

                    # Show explorer URL (but don't open browser automatically)
                    explorer_url = f"{self.explorer_url}/tx/{tx_hash}"
                    block_url = f"{self.explorer_url}/block/{final_height}"
                    self.root.after(0, lambda u=block_url: self.log(f"🔗 Block Explorer: {u}", "INFO"))
                    self.root.after(0, lambda u=explorer_url: self.log(f"🔗 Transaction Explorer: {u}", "INFO"))
    ...
    def update_stats(self):
        ...
        try:
            ...
            print(f"[STAT TAB UPDATE ERROR] {e}")
